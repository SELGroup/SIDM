...
class PartitionTree():
    ...
    def entropy(self,node_dict = None):
        ...
        for node_id,node in node_dict.items():
            if node.parent is not None:
                ...
                if (node_p_vol > 0.0) and (node_vol > 0.0):
    ...
    def node_entropy(self, id):
        node = self.tree_node[id]
        pid = node.parent
        node_vol = node.vol
        node_g = abs(node.g)
        node_p_vol = self.tree_node[pid].vol
        assert node_vol <= node_p_vol
        if node_vol != 0 and node_p_vol != 0:
            return abs((node_g / self.VOL) * math.log2(node_vol / node_p_vol))
        else:
            return 0.0
...
def cosine_similarity_nonzero(vector1, vector2):
    # 只保留非零元素
    nonzero_indices1 = vector1 != 0
    nonzero_indices2 = vector2 != 0
    nonzero_indices = nonzero_indices1 & nonzero_indices2
    v1 = vector1[nonzero_indices]
    v2 = vector2[nonzero_indices]
...
def custom_cosine_similarity(matrix):
    # 使用自定义函数计算余弦相似度矩阵
    return 1 - squareform(pdist(matrix, lambda u, v: cosine_similarity_nonzero(u, v)))

def si_abstract(data):
    cp_data = data.clone()
    rs_data = cp_data.reshape([-1, cp_data.shape[-1]]).cpu().detach().numpy()  # [batch_size, seq_len, data_dimmension] -> [batch_size * seq_len, data_dimmension]
    # 计算行向量间的余弦相似度
    cosine_similarity = custom_cosine_similarity(rs_data)
    for i in range(cosine_similarity.shape[0]):
        cosine_similarity[i, i] = 0.0
    filtered_similarity = np.where(cosine_similarity > 0.0, cosine_similarity, 0.0)
    # 构造编码树
    pt = PartitionTree(adj_matrix=filtered_similarity)
    pt.build_encoding_tree(k=9)
    id_set = set()
    for node in pt.tree_node.values():
        if node.children == None:
            id_set.add(node.parent)
    rep_dict = dict()
    for nid in id_set:
        node = pt.tree_node[nid]
        rep, ens = None, 0.0
        for cid in node.children:
            en = pt.node_entropy(cid)
            index_0, index_1 = int(cid/cp_data.shape[1]), int(cid % cp_data.shape[1])
            assert index_0 < cp_data.shape[0] and index_1 < cp_data.shape[1]
            if rep is None:
                rep = en * cp_data[int(cid/cp_data.shape[1]), int(cid % cp_data.shape[1])]
            else:
                rep = rep + en * cp_data[int(cid/cp_data.shape[1]), int(cid % cp_data.shape[1])]
            ens = ens + en
        rep = rep / ens
        rep_dict[nid] = rep
    return pt, rep_dict
